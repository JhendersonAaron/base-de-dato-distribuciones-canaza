...
from {{ ref('stg_comprobante') }} c
join {{ ref('stg_comprobante_detalle') }} cd
    on c.id_comprobante = cd.id_comprobante
join {{ ref('dim_fecha') }} df
    on c.fecha_emision = df.fecha
join {{ ref('dim_producto') }} dp
    on cd.id_producto = dp.id_producto
join {{ ref('dim_entidad') }} de
    on c.id_entidad = de.id_entidad
join {{ ref('dim_tipo_cpe') }} dtc
